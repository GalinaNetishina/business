...
class CompanyService(BaseService):
    ...
    async def create_company(self, company: CompanyRequest, admin) -> CompanyModel:
        """Create company."""
        return await self.uow.company.add_one_and_get_obj(**company.model_dump(), admin_id=admin.id)

    @transaction_mode
    async def get_companies(self) -> list[CompanyDB]:
        res = await self.uow.company.get_by_query_all()
        return list(map(lambda x: CompanyDB.model_validate(x, from_attributes=True), res ))

    @transaction_mode
    async def get_company(self, company_id) -> CompanyWithUsers:
        """Find company by ID with all users."""
        company: CompanyModel | None = await self.uow.company.get_company(
            company_id
        )
